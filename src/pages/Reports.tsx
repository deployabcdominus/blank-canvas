...
export default function Reports() {
  ...
  return (
    <PageTransition>
      <ResponsiveLayout title={t.landing.reports.title} subtitle={t.landing.reports.subtitle}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <Card className="glass-card border-white/10">
            <CardHeader className="flex flex-row items-center gap-3">
              <TrendingUp className="w-5 h-5 text-violet-400" />
              <CardTitle className="text-base font-bold">{t.landing.reports.salesValue}</CardTitle>
            </CardHeader>
          ...
          <Card className="glass-card border-white/10">
            <CardHeader className="flex flex-row items-center gap-3">
              ...
              <CardTitle className="text-base font-bold">{t.landing.reports.productionStatus}</CardTitle>
            </CardHeader>
            <CardContent className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          <MetricSmall label={t.landing.reports.metricLeads} value={leads.length} icon={Users} color="text-violet-400" />
          <MetricSmall label={t.landing.reports.metricProposals} value={proposals.filter(p => (p.status as string) === 'Enviada externamente').length} icon={FileText} color="text-blue-400" />
          <MetricSmall label={t.landing.reports.metricCompleted} value={orders.filter(o => o.closing_status === 'Closed').length} icon={CheckCircle2} color="text-emerald-400" />
          <MetricSmall label={t.landing.reports.metricConversion} value={`${leads.length ? Math.round((proposals.filter(p => p.status === 'Aprobada').length / leads.length) * 100) : 0}%`} icon={TrendingUp} color="text-pink-400" />
        </div>
