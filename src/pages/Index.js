...
import { ArrowRight, Check, Zap, Building, Star, ChevronRight, Shield, Monitor, Thermometer, Signpost, Wrench, PenTool, WifiOff, Twitter, Instagram, Linkedin, ClipboardCheck } from "lucide-react";
...
const MacBookMockup = () => {
    ...
    return (<div className="relative w-full max-w-5xl mx-auto" style={{ perspective: "100rem" }}>
      ...
      <motion.div initial={{ opacity: 0, y: 70, rotateX: 12 }} animate={{ opacity: 1, y: 0, rotateX: 4 }} transition={{ duration: 1.4, delay: 0.3, ease: [0.22, 1, 0.36, 1] }} className="relative">
          <div className="relative rounded-t-[1.5rem] sm:rounded-t-[2rem] bg-zinc-950 overflow-hidden border border-white/10" style={{
            aspectRatio: "16/9",
            width: "100%",
            height: "auto",
        ...
        ].map((kpi) => (<div key={kpi.label} className="rounded-lg border border-white/[0.04] bg-white/[0.015] p-2 flex flex-col justify-center">
        ...
        ].map((order, i) => (<div key={order.id} className={`flex items-center gap-1.5 md:gap-2 p-1 md:p-1.5 rounded bg-white/[0.02] border border-white/[0.03] ${isMobile && i >= 3 ? 'hidden' : ''}`}>
        ...
        ].map((tech, i) => (<div key={i} className={`flex items-center gap-1.5 md:gap-2 px-1 md:px-1.5 py-1 rounded bg-white/[0.02] border border-white/[0.03] ${isMobile && i >= 3 ? 'hidden' : ''}`}>
...
const Index = () => {
    ...
    return (<PageTransition>
      <div className="noise-bg min-h-screen bg-zinc-950 text-zinc-200 overflow-x-hidden scroll-smooth selection:bg-purple-500/20 selection:text-white">
        ...
        {/* ═══════════ INDUSTRIES GRID ═══════════ */}
        <section id="industries" className="py-24 md:py-32 px-5 relative z-20 bg-black border-t border-white/5">
        ...
        <FloatingCTA show={showFloatingCTA} onClick={() => {
