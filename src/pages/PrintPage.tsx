      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ background: "#f8fafc", padding: 4, borderRadius: 4, border: "1px solid #e2e8f0" }}>
              <QRCodeSVG value={`${window.location.origin}/verify/${order.id}`} size={32} />
            </div>
            <div>
              <div style={{ fontSize: 7, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                Verification ID
              </div>
              <div style={{ fontSize: 8, color: "#64748b", fontFamily: "'JetBrains Mono', monospace" }}>
                {order.id.split('-')[0].toUpperCase()}-{woNumber.split('-').pop()}
              </div>
              <div style={{ fontSize: 7, color: "#94a3b8", marginTop: 1 }}>
                Scan to verify digital signature and document authenticity
              </div>
            </div>
          </div>
          ...
          <div style={{ textAlign: "right" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4, justifyContent: "flex-end" }}>
              {order.qc_signature_url && (
                <div style={{
                  display: "flex", alignItems: "center", gap: 1, fontSize: 7,
                  color: "#16a34a", fontWeight: 700, background: "#f0fdf4",
                  padding: "1px 6px", borderRadius: 10, border: "1px solid #bbf7d0"
                }}>
                  <Shield size={8} /> DIGITAL SIGNATURE CERTIFIED
                </div>
              )}
            </div>
            <div style={{ fontSize: 8, color: "#94a3b8" }}>
              Generated by Sign Flow Enterprise — {format(new Date(), "MMM dd, yyyy HH:mm")}
            </div>
          </div>
        </div>
      </div>
