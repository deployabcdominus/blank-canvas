...
import { RotateCcw, Save, Settings as SettingsIcon, User, Mail, Building2, Calendar, FolderOpen, Shield, KeyRound, Plug, Bell, List, Moon, CreditCard, Upload, Loader2, ImageIcon, Trash2, Globe, ExternalLink, Zap } from "lucide-react";
...
export default function Settings() {
    ...
    const updateSettings = (updates) => updateSettingsMutation.mutate(updates);
    ...
    useEffect(() => {
        ...
        if (company) {
            setNetworkEnabled(company.enable_network_index ?? false);
            setNetworkBasePath(company.network_base_path ?? '');
        }
    }, [company]);
    useEffect(() => {
    ...
    return (<ResponsiveLayout title={t.settings.title} subtitle={t.settings.subtitle} icon={SettingsIcon}>
      <Tabs value={activeTab} onValueChange={(v) => setSearchParams({ tab: v })} className="w-full">
         <TabsList className="mb-6">
          <TabsTrigger value="perfil">
           ...
           {isAdmin && !isSuperadmin && (<TabsTrigger value="organizacion">
              <Building2 className="w-4 h-4 mr-2"/>
              {t.settings.tabs.organization}
            </TabsTrigger>)}
          {isAdmin && !isSuperadmin && (<TabsTrigger value="storage">
              <FolderOpen className="w-4 h-4 mr-2"/>
              {t.settings.tabs.storage}
            </TabsTrigger>)}
           {isAdmin && (<TabsTrigger value="configuracion">
              <SettingsIcon className="w-4 h-4 mr-2"/>
              {t.settings.tabs.configuration}
            </TabsTrigger>)}
          {isAdmin && !isSuperadmin && (<TabsTrigger value="domains">
              <Globe className="w-4 h-4 mr-2"/>
              {t.settings.tabs.domains || "Custom Domains"}
            </TabsTrigger>)}
          {isAdmin && !isSuperadmin && (<TabsTrigger value="catalogos">
              <List className="w-4 h-4 mr-2"/>
              {t.settings.tabs.catalogs}
            </TabsTrigger>)}
           {isAdmin && !isSuperadmin && (<TabsTrigger value="integraciones">
              <Plug className="w-4 h-4 mr-2"/>
              {t.settings.tabs.integrations}
            </TabsTrigger>)}
          {isAdmin && !isSuperadmin && (<TabsTrigger value="notifications">
              <Bell className="w-4 h-4 mr-2"/>
              {t.settings.tabs.notifications}
            </TabsTrigger>)}
          {isAdmin && !isSuperadmin && (<TabsTrigger value="suscripcion">
              <CreditCard className="w-4 h-4 mr-2"/>
              {t.settings.tabs.subscription}
            </TabsTrigger>)}
          {isAdmin && !isSuperadmin && (<TabsTrigger value="audit">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder={t.settings.profile.fullNamePlaceholder}/>
                    <Button size="sm" onClick={handleSaveName} disabled={savingName || !fullName.trim() || fullName.trim() === (user?.user_metadata?.full_name || '')} className="flex items-center gap-2">
                  ...
                  {isSuperadmin ? (<div className="space-y-2">
                      ...
                      <Input value={company?.name || t.settings.profile.notAvailable} readOnly className="glass bg-muted/50"/>
                ...
                {!isSuperadmin && role && (<div className="space-y-2">
                    <Label className="flex items-center gap-2 text-muted-foreground">
                      <Shield className="w-4 h-4"/>
            ...
            borderBottom: "1px solid rgba(255,255,255,0.05)",
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input value={user?.email || t.settings.profile.notAvailable} readOnly className="glass bg-muted/50"/>
                  </div>
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <div className="flex items-center gap-3">
                      <Input type="password" value="••••••••••••" readOnly className="glass bg-muted/50 max-w-xs"/>
                      <Button variant="outline" size="sm" onClick={handleResetPassword} disabled={resetSent} className="flex items-center gap-2 whitespace-nowrap">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={user?.created_at ? new Date(user.created_at).toLocaleDateString(isEn ? 'en-US' : 'es-ES', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : t.settings.profile.notAvailable} readOnly className="glass bg-muted/50"/>
                </div>

                <div className="space-y-2">
                  <Label className="text-muted-foreground">{t.settings.profile.userId}</Label>
                  <Input value={user?.id || t.settings.profile.notAvailable} readOnly className="glass bg-muted/50 text-xs font-mono"/>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        {isAdmin && !isSuperadmin && (<TabsContent value="domains">
            <div className="grid gap-6">
                  ...
                  <div className="p-4 rounded-xl bg-white/[0.03] border border-white/5 space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between p-3 rounded-lg bg-primary/5 border border-primary/10">
                        ...
                        <Button size="sm" className="bg-primary hover:bg-primary/90 text-primary-foreground text-xs h-8" onClick={() => {
                ...
                <CardContent className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input value={company?.email || ""} onChange={(e) => updateCompanySettings({ email: e.target.value })} placeholder="contact@company.com" className="bg-white/[0.03] border-white/10 h-9 text-sm"/>
                    </div>
                    <div className="space-y-2">
                      <Label>{t.settings.domains.publicPhone}</Label>
                      <Input value={company?.phone || ""} onChange={(e) => updateCompanySettings({ phone: e.target.value })} placeholder="+1 (555) 000-0000" className="bg-white/[0.03] border-white/10 h-9 text-sm"/>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label>{t.settings.domains.businessAddress}</Label>
                    <Input value={company?.address || ""} onChange={(e) => updateCompanySettings({ address: e.target.value })} placeholder="123 Sign St, Production City" className="bg-white/[0.03] border-white/10 h-9 text-sm"/>
                  </div>
                  <div className="flex items-center justify-between p-3 rounded-lg border border-white/5 bg-white/[0.02]">
                    ...
                    <Switch checked={company?.auto_create_production_orders} onCheckedChange={(v) => updateCompanySettings({ auto_create_production_orders: v })}/>
                  </div>
                </CardContent>
              </Card>
            </div>
        ...
        {isAdmin && !isSuperadmin && (<TabsContent value="organizacion">
            <div className="grid gap-6">
              <Card>
                <CardHeader>
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-4">
                      {company?.logo_url ? (<div className="relative group">
                          <img src={company.logo_url} alt={company.name} className="h-16 max-w-[200px] object-contain rounded-lg border border-border p-2 bg-muted/30"/>
                    ...
                    try {
                        ...
                        </div>) : (<div className="h-16 w-32 rounded-lg border border-dashed border-border flex items-center justify-center bg-muted/20">
                          <ImageIcon className="w-6 h-6 text-muted-foreground"/>
                        </div>)}
                      <div>
                        <Button variant="outline" size="sm" disabled={uploadingLogo} onClick={() => document.getElementById('company-logo-input')?.click()} className="flex items-center gap-2">
                        ...
                        <input id="company-logo-input" type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={async (e) => {
                const file = e.target.files?.[0];
                if (!file || !company || !user)
                    return;
                ...
                try {
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="org-name" value={orgName} onChange={(e) => setOrgName(e.target.value)} placeholder={t.settings.org.namePlaceholder}/>
                  </div>
                  <Button onClick={async () => {
                if (!orgName.trim())
                    return;
            ...
            }} disabled={savingOrg || !orgName.trim() || orgName.trim() === company?.name} className="flex items-center gap-2">
        ...
        {isAdmin && !isSuperadmin && (<TabsContent value="storage">
            <div className="grid gap-6">
              <Card>
                <CardHeader>
                ...
                <CardContent className="space-y-6">
                  ...
                  {networkEnabled && (<div className="space-y-2">
                      <Label>{t.settings.storage.networkPath}</Label>
                      <Input value={networkBasePath} onChange={e => setNetworkBasePath(e.target.value)} placeholder="\\dropbox\The Sign Space\Projects\"/>
                      <p className="text-xs text-muted-foreground">
            ...
            }} disabled={savingStorage} className="flex items-center gap-2">
                    <Save className="w-4 h-4"/>
        ...
        {isAdmin && (<TabsContent value="configuracion">
            <div className="grid gap-6">
              <Card>
                <CardHeader>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch checked={formData.glassEffect} onCheckedChange={(checked) => setFormData(prev => ({ ...prev, glassEffect: checked }))}/>
                  </div>
        ...
        {isAdmin && !isSuperadmin && (<TabsContent value="catalogos">
            <div className="space-y-4">
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <CatalogManager type="lead_service" title={isEn ? "Services" : "Servicios"} description={isEn ? "Types of work you offer. Shown when creating a lead." : "Tipos de trabajo que ofreces. Aparece al crear un lead."}/>
                <CatalogManager type="lead_source" title={isEn ? "Lead sources" : "Fuentes de leads"} description={isEn ? "How the client found you. Shown when creating a lead." : "Cómo te encontró el cliente. Aparece al crear un lead."}/>
                <CatalogManager type="lead_status" title={isEn ? "Lead statuses" : "Estados de leads"} description={isEn ? "Stages of the sales process." : "Etapas del proceso de venta."} hasColor/>
                <CatalogManager type="order_status" title={isEn ? "Order statuses" : "Estados de órdenes"} description={isEn ? "Stages of the production and installation process." : "Etapas del proceso de producción e instalación."} hasColor/>
                <CatalogManager type="material_type" title={isEn ? "Material types" : "Tipos de materiales"} description={isEn ? "Materials available when building a proposal." : "Materiales disponibles al armar una propuesta."}/>
              </div>
            </div>
          </TabsContent>)}
        {isAdmin && !isSuperadmin && (<TabsContent value="integraciones">
            <IntegrationsCards />
          </TabsContent>)}
        {isAdmin && !isSuperadmin && (<TabsContent value="suscripcion">
            <div className="grid gap-6">
              {!planLimits.loading && (<Card>
                  <CardHeader>
                  ...
                  <CardContent className="space-y-5">
                    {[
                    { key: "work_orders", label: t.settings.planUsage.workOrders },
                    { key: "leads", label: t.settings.planUsage.leads },
                    { key: "users", label: t.settings.planUsage.users },
                    { key: "proposals", label: t.settings.planUsage.proposals },
                ].map(({ key, label }) => {
                    ...
                    return (<div key={key} className="space-y-1.5">
                          ...
                          {!lim.isUnlimited && (<div className="h-1.5 w-full rounded-full bg-white/10 overflow-hidden">
                              <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: barColor }}/>
                            </div>)}
                        </div>);
                })}
                    {planLimits.planName !== "Empresarial" && (<div className="pt-2">
                        <Button variant="outline" size="sm" onClick={() => { }} className="border-violet-500/30 text-violet-300 hover:bg-violet-500/10">
        ...
        {isAdmin && !isSuperadmin && (<TabsContent value="notifications">
            <div className="grid gap-6">
            ...
            ].map((channel) => (<div key={channel.key} className="flex items-center justify-between p-3 rounded-lg border border-white/5 bg-white/[0.02]">
                      <div className="space-y-0.5">
        ...
        {isAdmin && !isSuperadmin && (<TabsContent value="audit">
            <Card className="glass border-white/10">
              <CardHeader>
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-primary/10">
