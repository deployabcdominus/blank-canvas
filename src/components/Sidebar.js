...
import { platformItems, tenantGroups, utilityItems, } from "@/constants/navigation";
export const Sidebar = memo(() => {
    ...
    if (roleLoading)
        return null;
    return createPortal(<motion.aside initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }} className="fixed left-0 top-0 bottom-0 z-[100] hidden md:flex flex-col border-r border-white/[0.04] bg-zinc-950/80 backdrop-blur-3xl w-[72px] lg:w-[280px] p-4 lg:p-6 shadow-2xl transition-all duration-300 isolate h-[100dvh]" role="navigation" aria-label="Menu lateral principal">
      {/* Logo Section */}
      ...
      {isSuperadmin ? (<SidebarPlatformNav items={platformItems} location={location} industryLabels={industryLabels} platformLabel={t.nav.platform} companyId={companyId} t={t}/>) : (<SidebarTenantNav groups={localizedGroups} utilityItems={utilityItems} location={location} role={role} industryLabels={industryLabels} isAdmin={isAdmin} adjustmentsLabel={t.nav.adjustments} companyId={companyId} t={t}/>)}
      </div>

      {/* Footer Section: Plan & User */}
      <div className="mt-auto space-y-6">
        {/* Plan Upgrade Card (Ref Image Reference) */}
        {!isSuperadmin && isAdmin && (<div className="hidden lg:block p-4 rounded-2xl bg-white/[0.03] border border-white/[0.05] relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <button onClick={() => navigate("/settings?tab=billing")} className="w-full py-2 px-3 rounded-xl bg-white/[0.05] hover:bg-white/[0.08] text-white text-[12px] font-semibold transition-colors border border-white/[0.05]">
                Manage Plan
              </button>
            </div>
          </div>)}

        <SidebarUserFooter avatarUrl={avatarUrl} fullName={fullName} email={email} initials={initials} isSuperadmin={isSuperadmin} isAdmin={isAdmin} onLogout={async () => { await signOut(); navigate("/login"); }} onNavigate={navigate} profileLabel={t.nav.profile} settingsLabel={t.nav.settings} logoutLabel={t.nav.logout}/>
      </div>
    </motion.aside>, document.body);
});
Sidebar.displayName = "Sidebar";
/* ─── Helpers ─── */
const isActivePath = (location, path) => location.pathname + location.search === path || location.pathname === path;
const getLabel = (item, industryLabels, t) => {
...
const SidebarNavItem = memo(({ item, location, industryLabels, companyId, t }) => {
        ...
        if (item.path === "/leads") {
            ...
            queryClient.prefetchQuery({
                ...
                queryFn: async () => {
                    ...
                    const orderProposalIds = new Set((res.orders || []).map(o => o.proposal_id).filter((id) => !!id));
                    return {
    ...
    return (<NavLink to={item.path} onMouseEnter={handleMouseEnter} className={({ isActive }) => `group relative flex items-center transition-all duration-300 justify-center lg:justify-start rounded-2xl lg:rounded-xl p-2.5 lg:px-4 lg:py-3 ${isActive
            ? "bg-primary/10 border border-primary/20 text-white font-semibold shadow-[0_0_20px_rgba(139,92,246,0.15)]"
            : "border border-transparent text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.03]"}`} title={label} aria-current={active ? "page" : undefined}>
      <item.icon className={`flex-shrink-0 transition-colors duration-300 ${active ? "text-primary" : "text-zinc-500 group-hover:text-zinc-300"}`} size={20} strokeWidth={active ? 2 : 1.5} aria-hidden="true"/>
      <span className="hidden lg:block text-[14px] leading-tight ml-3.5 truncate">
        {label}
      </span>
      {active && (<motion.div layoutId="sidebar-active-dot" className="absolute right-2 w-1.5 h-1.5 rounded-full bg-primary hidden lg:block" transition={{ type: "spring", stiffness: 400, damping: 28 }}/>)}
    </NavLink>);
});
      ...
      <div className="hidden lg:block">
        <Collapsible open={isOpen} onOpenChange={onToggle}>
          <CollapsibleTrigger className="flex items-center justify-between w-full px-4 py-2 group cursor-pointer select-none rounded-xl hover:bg-white/[0.02] transition-colors">
            ...
            <ChevronRight className={`text-zinc-700 transition-transform duration-200 group-hover:text-zinc-500 ${isOpen ? "rotate-90" : ""}`} size={14} strokeWidth={2.5}/>
          </CollapsibleTrigger>
          <CollapsibleContent>
            <AnimatePresence>
              {isOpen && (<motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.18, ease: [0.22, 1, 0.36, 1] }} className="space-y-0.5 mt-1">
                  {visibleItems.map(item => (<SidebarNavItem key={item.path} item={item} location={location} industryLabels={industryLabels} companyId={companyId} t={t}/>))}
                </motion.div>)}
            </AnimatePresence>
          </CollapsibleContent>
        </Collapsible>
...
function SidebarPlatformNav({ items, location, industryLabels, platformLabel, companyId, t }) {
    return (<nav className="space-y-1 min-h-0">
      <p className="hidden lg:block px-4 py-2 text-[11px] font-bold uppercase tracking-[0.2em] text-zinc-600">
        {platformLabel || "Platform"}
      </p>
      {items.map(item => (<SidebarNavItem key={item.path} item={item} location={location} industryLabels={industryLabels} companyId={companyId} t={t}/>))}
    </nav>);
}
/* ─── Tenant Nav ─── */
function SidebarTenantNav({ groups, utilityItems: utils, location, role, industryLabels, isAdmin, adjustmentsLabel, companyId, t }) {
    const activeGroupIdx = groups.findIndex(g => g.items.some(i => isActivePath(location, i.path)));
    ...
    return (<>
      <nav className="min-h-0 space-y-3 lg:space-y-4">
        {principalItems.length > 0 && (<div className="space-y-1">
            ...
            {principalItems.map(item => (<SidebarNavItem key={item.path} item={item} location={location} industryLabels={industryLabels} companyId={companyId} t={t}/>))}
          </div>)}

        {collapsibleGroups.map((group, idx) => (<SidebarCollapsibleGroup key={group.groupLabel} group={group} isOpen={openGroup === idx + 1} onToggle={() => setOpenGroup(openGroup === idx + 1 ? -1 : idx + 1)} location={location} role={role} industryLabels={industryLabels} companyId={companyId} t={t}/>))}
      </nav>

      {visibleUtils.length > 0 && (<div className="pt-3 mt-2 border-t border-white/[0.04] space-y-0.5">
          ...
          {visibleUtils.map(item => (<SidebarNavItem key={item.path} item={item} location={location} industryLabels={industryLabels} companyId={companyId} t={t}/>))}
        </div>)}
    </>);
}
/* ─── User Footer ─── */
function SidebarUserFooter({ avatarUrl, fullName, email, initials, isSuperadmin, isAdmin, onLogout, onNavigate, profileLabel, settingsLabel, logoutLabel }) {
    return (<div className="flex-shrink-0 pt-4 border-t border-white/[0.04]">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="flex items-center rounded-2xl transition-all duration-200 hover:bg-white/[0.03] w-full justify-center lg:justify-start lg:gap-3 lg:px-2.5 lg:py-2.5 p-2" title="Mi Perfil" aria-label="Menú del usuario">
        ...
        <DropdownMenuContent align="end" side="right" className="glass-card w-56 z-50" sideOffset={8}>
          <div className="p-2.5">
            ...
            {isSuperadmin && (<Badge variant="outline" className="mt-1.5 text-xs border-primary/30 text-primary font-bold">
          ...
          {isAdmin && (<DropdownMenuItem onClick={() => onNavigate("/settings")} className="min-h-[40px] text-zinc-300 hover:text-white">
