...
export const LeadPipelineStepper = ({ currentStage }: LeadPipelineStepperProps) => {
  ...
  return (
    <div className="w-full">
      <div className="relative flex items-center justify-between rounded-2xl border border-zinc-100 bg-zinc-50/50 px-3 py-2">
        {/* Connector line */}
            ...
            <div key={stage.label} className="relative z-10 flex flex-col items-center gap-0.5">
              <motion.div
                ...
                className={`
                  ...
                  ${active ? "bg-primary/10 text-primary ring-1.5 ring-primary/30 shadow-[0_0_12px_hsl(var(--primary)/0.15)]" : ""}
                  ${future ? "bg-zinc-100 text-zinc-300" : ""}
                `}
              >
                {done ? (
              ...
              <span
                className={`text-[9px] font-medium leading-tight transition-colors ${
                  done ? "text-primary" : active ? "text-zinc-900" : "text-zinc-300"
                }`}
              >
                {stage.label}
              </span>
