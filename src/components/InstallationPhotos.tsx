...
export const InstallationPhotos: React.FC<InstallationPhotosProps> = ({ installationId, isReadOnly = false }) => {
  ...
  const handleFiles = useCallback(async (files: FileList | File[]) => {
    ...
    if (fileArray.length === 0) {
      toast({ title: "Error", description: "Seleccione solo archivos de imagen.", variant: "destructive" });
      return;
    }
    if (!companyId) {
      toast({ title: "Error", description: "No se encontró la empresa.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      setPhotos(updated);
      toast({ title: "Fotos agregadas", description: `${newUrls.length} foto(s) subida(s) con éxito.` });
    } catch (error) {
      console.error('Error uploading photos:', error);
      toast({ title: "Error", description: "Error al subir las imágenes.", variant: "destructive" });
    } finally {
      setUploading(false);
    }
  }, [photos, companyId, installationId]);
  ...
  const clearAll = async () => {
    try {
      ...
      setPhotos([]);
      toast({ title: "Fotos eliminadas", description: "Todas las fotos fueron eliminadas." });
    } catch (error) {
      console.error('Error clearing photos:', error);
      toast({ title: "Error", description: "Error al limpiar las fotos.", variant: "destructive" });
    }
  };
    ...
    <div>
        ...
        <div className="mt-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {photos.map((url, i) => (
            <div key={i} className="relative group">
              <img src={url} alt="Foto de ejecución" className="w-full h-24 object-cover rounded-lg border border-border" />
              {!isReadOnly && (
      ...
      {!isReadOnly && (
        ...
        >
          ...
          <p className="text-xs text-muted-foreground mb-4">Arrastre y suelte, pegue (Ctrl+V) o haga clic</p>
