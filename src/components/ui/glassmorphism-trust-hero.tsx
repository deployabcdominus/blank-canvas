...
const StatItem = ({ value, label }: { value: string; label: string }) => (
  <div className="flex flex-col">
    ...
    <span className="text-[10px] text-white/50 uppercase tracking-widest leading-none mt-1">{label}</span>
  </div>
);
      ...
      <div className="relative z-20 w-full max-w-7xl px-6 lg:px-8">
        <div className="grid lg:grid-cols-[1.2fr_0.8fr] gap-12 lg:gap-20 items-center">
          ...
          <div className="relative animate-fade-in delay-200 hidden lg:block">
            {/* Stats Card */}
            <div className="relative group overflow-hidden rounded-[2.5rem] border border-white/10 bg-zinc-900/40 p-10 backdrop-blur-2xl transition-all hover:border-white/20">
              ...
              <div className="relative z-10 flex flex-col space-y-10">
                <div className="flex items-start justify-between">
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-600 to-fuchsia-600 shadow-lg shadow-violet-600/20">
                    <Target className="text-white" size={32} />
                  </div>

                  <div className="text-right">
                    <div className="text-5xl font-black text-white tracking-tighter">500+</div>
                    <div className="text-[10px] font-bold text-white/30 uppercase tracking-[0.3em] mt-1">{locale === 'es' ? 'Empresas Activas' : 'Active Businesses'}</div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between text-sm font-bold uppercase tracking-widest">
                    ...
                    <span className="text-violet-400">99.8%</span>
                  </div>
                  <div className="h-2.5 w-full overflow-hidden rounded-full bg-white/5">
                    <div className="h-full w-[99%] bg-gradient-to-r from-violet-600 to-fuchsia-600" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-8">
                  ...
                  <StatItem value="150+" label={locale === 'es' ? 'Proyectos' : 'Projects'} />
                  <StatItem value="4.9" label={locale === 'es' ? 'Rating' : 'Rating'} />
                </div>

                {/* Tag Pills */}
                <div className="flex flex-wrap gap-3 pt-2">
                  <div className="flex items-center space-x-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-[10px] font-black tracking-widest text-violet-400">
                  ...
                  <div className="flex items-center space-x-2 rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-[10px] font-black tracking-widest text-white/40 uppercase">
                    <Crown size={12} className="text-fuchsia-500/50" />
                    <span>{locale === 'es' ? 'Empresarial' : 'Enterprise'}</span>
                  </div>
                </div>
              </div>
            </div>
