...
/* ── Status helpers ── */
const statusLabel = (s, labels) => s === "Completed" ? labels.completed : s === "In Progress" ? labels.inProgress : labels.scheduled;
const statusBadge = (s) => {
    if (s === "Completed")
...
export const GeoHeatmap = ({ installations }) => {
    ...
    const [geoMap, setGeoMap] = useState(new Map());
    useEffect(() => {
        ...
        async function load() {
            setGeocoding(true);
            const results = await batchGeocode(installations.slice(0, 30), inst => inst.address || undefined);
            if (!cancelled) {
                setGeoMap(results);
                setGeocoding(false);
            }
        ...
        <div style="font-family:Inter,system-ui,sans-serif;">
          ...
          <button onclick="window.__signflow_nav_installation && window.__signflow_nav_installation()" style="width:100%;margin-top:10px;padding:6px 0;font-size:12px;color:#f97316;background:rgba(249,115,22,0.08);border:1px solid rgba(249,115,22,0.2);border-radius:8px;cursor:pointer;font-family:inherit;">
            ...
            const marker = L.marker([m.lat, m.lng], { icon })
                ...
                .addTo(map);
            marker.on("popupopen", () => {
                window.__signflow_nav_installation = () => navigate("/installation");
            });
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.45, duration: 0.5 }} className="rounded-2xl border border-white/[0.06] bg-zinc-950/40 backdrop-blur-2xl p-5 transition-all duration-300 hover:border-white/[0.12] hover:bg-zinc-950/50 relative overflow-hidden col-span-1 lg:col-span-2">
      ...
      <div className="relative w-full h-[320px] rounded-xl overflow-hidden border border-white/[0.04]">
        {geocoding && (<div className="absolute inset-0 flex items-center justify-center z-[1000] bg-zinc-950/60 backdrop-blur-sm">
        ...
        {!geocoding && markers.length === 0 && (<div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-center p-4 z-[1000]">
