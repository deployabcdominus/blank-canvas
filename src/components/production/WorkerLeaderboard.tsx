  ...
  if (board.length === 0) {
    return (
      <div className="rounded-2xl border border-border bg-card p-5 text-center">
        ...
        <p className="text-sm text-muted-foreground">{t.production.operatorStation.noActivity}</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="font-black text-foreground">{t.production.operatorStation.leaderboardTitle}</h3>
      </div>
      <p className="text-xs text-muted-foreground mb-4">
        {t.production.operatorStation.leaderboardSubtitle}
      </p>
        ...
        {board.map((entry, i) => (
          <div key={entry.user_id} className={`flex items-center gap-3 p-3 rounded-xl ${i === 0 ? "bg-amber-500/10 border border-amber-500/20" : "bg-muted/50"}`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-foreground truncate">{entry.full_name ?? (locale === "en" ? "Worker" : "Operario")}</div>
              <div className="text-xs text-muted-foreground">{t.production.operatorStation.level} {entry.level} · {entry.level_title}</div>
