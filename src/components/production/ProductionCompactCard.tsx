...
import {
  ...
  MoreHorizontal, Eye, Printer, Share2, Zap, Calendar,
} from "lucide-react";

const STATUS_MAP: Record<string, { color: string; icon: React.ReactNode }> = {
  "Materiales Pedidos": { color: "bg-lavender text-lavender-foreground", icon: <Package className="w-3 h-3" /> },
  "En Producción": { color: "bg-soft-blue text-soft-blue-foreground", icon: <Wrench className="w-3 h-3" /> },
  "Control de Calidad": { color: "bg-pale-pink text-pale-pink-foreground", icon: <AlertTriangle className="w-3 h-3" /> },
  "Producido": { color: "bg-mint text-mint-foreground", icon: <CheckCircle className="w-3 h-3" /> },
};
...
export function ProductionCompactCard({ order, index, onOpen, onMarkBuilt }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{order.materials.length} materiales</span>
